...
## Manager 嵌套列表代码块缺少语言标识

**问题描述**: 在 Manager 窗口中, 嵌套列表内的代码块复制时无法正确识别语言标识符.

**影响范围**:
- Manager 窗口中的嵌套列表 (有序/无序)
- 列表项内包含的代码块

**原因**: Manager 窗口的 DOM 结构与侧边栏不同, 代码块元素可能不是 `<li>` 的直接子元素, 导致语言标识查找失败.

**备注**: 侧边栏 (cascade-panel) 的嵌套列表代码块已正常支持. Manager 窗口由于缺少浏览器开发者工具, 暂时无法调试具体 DOM 结构.
